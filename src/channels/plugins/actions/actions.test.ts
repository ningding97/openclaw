...
describe("telegramMessageActions", () => {
  ...
  it("prefers lastUserMessageId over currentMessageId for reactions", async () => {
    const cfg = telegramCfg();
    ...
    await telegramMessageActions.handleAction?.({
      ...
      cfg,
      accountId: undefined,
      toolContext: { currentMessageId: "9999", lastUserMessageId: "7777" },
    });
    ...
    const callPayload = call as Record<string, unknown>;
    expect(callPayload.action).toBe("react");
    expect(String(callPayload.messageId)).toBe("7777");
  });
